...
if(length(args)!=10){
  ...
  message("\nUSAGE:\tRscript PrepareTargetPositions.R -bed -fasta -dbsnp -outdir -n\n")
  cat(paste("\t-bed\t\tTarget regions in BED format",
            "\t-fasta\t\tReference Genome in fasta format used to align BAM files",
            "\t-dbsnp\t\tFolder named dbSNP_NO_clinvar_somatic_folder_ByChromosome generated by utility/dbSNPPER.R",
            "\t-outdir\t\toutput folder in which subfolder TargetPositions will be created",
            "\t-n\t\tNumber of threads",sep="\n"),"\n\n")  
  quit()
}
args = data.frame(flag=args[c(1,3,5,7,9)],input=args[c(2,4,6,8,10)],stringsAsFactors = F)
targetbed <- as.character(args$input[which(args$flag=='-bed')])
ReferenceFasta = as.character(args$input[which(args$flag=='-fasta')]) 
dbsnpfolder = as.character(args$input[which(args$flag=='-dbsnp')])
outdir = as.character(args$input[which(args$flag=='-outdir')])
Ncores = as.integer(args$input[which(args$flag=='-n')])
...
getpos <- function(id,bed,fasta){
  ...
  #sequence = unlist(strsplit(fasta[which(fasta$chr==chrom & fasta$start==start-1 & fasta$end==end-1),4],split = "*"))
...
for(chrom in CHR){
  ...
  dbsnpfile = grep(dbsnp_chroms,pattern = paste0("_chr",gsub(chrom,pattern = "chr",replacement = ""),".vcf"),value = T)
  ...
  # Get SNPs that are not clinvar somatic
